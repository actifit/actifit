...
public class PostAdapter extends ArrayAdapter<SingleHivePostModel> {
    ...
    Markwon markwon;
    private Set<String> translatingPermlinks = new HashSet<>();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        try {
            ...
            if (postEntry.commentsExpanded){
                ...
                if (!this.isComment) { expandButton.setVisibility(View.GONE); retractButton.setVisibility(View.VISIBLE); }
            }else{
                commentsList.setVisibility(View.GONE);
                if (!this.isComment) { expandButton.setVisibility(View.VISIBLE); retractButton.setVisibility(View.GONE); }
            }
            View finalConvertView = convertView;
            replyButton.setOnClickListener(view -> { /* Your existing code */ });
            upvoteButton.setOnClickListener(view ->{ /* Your existing code */ });
            commentButton.setOnClickListener(view -> { /* Your existing code */ });
            ...
            try {
                ...
                if (activityTypeStr.isEmpty()) { activityTypeContainer.setVisibility(View.GONE); } else { activityTypeContainer.setVisibility(View.VISIBLE); }
                String activityCountStr = postEntry.getActivityCount(true);
                if (Objects.equals(activityCountStr, "")) { activityCountContainer.setVisibility(View.GONE); } else {
                    ...
                    if (actiCountNo >= 10000) { activityCount.setTextColor(getContext().getResources().getColor(R.color.actifitDarkGreen));
                    } else if (actiCountNo >= 5000) { activityCount.setTextColor(getContext().getResources().getColor(R.color.actifitRed));
                    } else { activityCount.setTextColor(getContext().getResources().getColor(android.R.color.tab_indicator_text)); }
                }
                payoutVal.setText(Html.fromHtml(grabPostPayout(postEntry) + (isPaid(postEntry) ? checkMark : hourglass)));
                if (isPaid(postEntry)) { payoutVal.setTextColor(colorSuccess); } else { payoutVal.setTextColor(colorPending); }
            ...
            this.markwon = Markwon.builder(ctx).usePlugin(HtmlPlugin.create()).usePlugin(new AbstractMarkwonPlugin() {@Override public void configureConfiguration(MarkwonConfiguration.Builder builder) { builder.linkResolver((view, link) -> { Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(link)); intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); if (intent.resolveActivity(view.getContext().getPackageManager()) != null) { view.getContext().startActivity(intent); } }); }}).usePlugin(PicassoImagesPlugin.create(new PicassoImagesPlugin.PicassoStore() { @NonNull @Override public RequestCreator load(@NonNull AsyncDrawable drawable) { int desiredWidth = body.getWidth(); if (desiredWidth <= 0) { return Picasso.get().load(drawable.getDestination()).tag(drawable); } return Picasso.get().load(drawable.getDestination()).resize(desiredWidth, 0).tag(drawable); } @Override public void cancel(@NonNull AsyncDrawable drawable) { Picasso.get().cancelTag(drawable); } })).build();
            renderContent(postEntry, body, progressBarBody);
            commentsList = convertView.findViewById(R.id.comments_list);
            commentsList.setOnTouchListener((v, event) -> { ListView parentListView = Utils.findParentListView(v); if (parentListView != null) { parentListView.requestDisallowInterceptTouchEvent(event.getAction() != MotionEvent.ACTION_UP && event.getAction() != MotionEvent.ACTION_CANCEL); } return false; });
            if (!this.isComment) {
                title.setVisibility(View.VISIBLE);
                afitRewards.setVisibility(View.VISIBLE);
                String fetchedImageUrl = "";
                try { JSONObject jsonMetadata = postEntry.json_metadata; if (jsonMetadata.has("image")) { JSONArray imageArray = jsonMetadata.getJSONArray("image"); if (imageArray.length() > 0) { for (int j = 0; j < imageArray.length(); j++) { fetchedImageUrl = imageArray.getString(j); if (fetchedImageUrl.startsWith("http")) break; } } } } catch (JSONException e) { e.printStackTrace(); }
                final String mainImageUrl = fetchedImageUrl;
                uiHandler.post(() -> { try { if (!mainImageUrl.isEmpty()) { Picasso.get().load(mainImageUrl).error(R.drawable.ic_launcher_background).fit().centerCrop().into(mainImage); mainImage.setVisibility(View.VISIBLE); } else { mainImage.setVisibility(View.GONE); } } catch (Exception err) { System.out.println(err); mainImage.setVisibility(View.GONE); } });
                afitLogo.setVisibility(View.VISIBLE);
            } else { mainImage.setVisibility(View.GONE); title.setVisibility(View.GONE); afitRewards.setVisibility(View.GONE); expandButton.setVisibility(View.GONE); afitLogo.setVisibility(View.GONE); }
        } catch(Exception exp) { exp.printStackTrace(); }
        return convertView;
    }

    ArrayList<SingleHivePostModel> loadComments(SingleHivePostModel postEntry){
        HiveRequests hiveReq = new HiveRequests(ctx); ArrayList<SingleHivePostModel> commentList = new ArrayList<>(); try { JSONObject params = new JSONObject(); params.put("author", postEntry.author); params.put("permlink", postEntry.permlink); JSONArray result = hiveReq.getComments(params); for (int i = 0; i < result.length(); i++) { SingleHivePostModel commentEntry = new SingleHivePostModel((result.getJSONObject(i)), ctx); commentList.add(commentEntry); } } catch (Exception ex) { ex.printStackTrace(); } return commentList;
    }

    void shareSocial(SingleHivePostModel postEntry){
        Intent sharingIntent = new Intent(Intent.ACTION_SEND); sharingIntent.setType("text/plain"); sharingIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); String shareSubject = ctx.getString(R.string.post_title_other); String shareBody = ctx.getString(R.string.post_description) + ctx.getString(R.string.post_title_other) + " " + ctx.getString(R.string.actifit_url) + postEntry.author + "/" + postEntry.permlink; sharingIntent.putExtra(Intent.EXTRA_SUBJECT, shareSubject); sharingIntent.putExtra(Intent.EXTRA_TEXT, shareBody); socialActivContext.startActivity(Intent.createChooser(sharingIntent, ctx.getString(R.string.share_via)));
    }
    ...
    void handleTranslation(SingleHivePostModel postEntry, TextView body, RelativeLayout progressBarBody, Activity activity) {
        if (translatingPermlinks.contains(postEntry.permlink)) {
            return;
        }
        if (!postEntry.isTranslated) {
            if (postEntry.translatedText != null && !postEntry.translatedText.isEmpty()) {
                postEntry.isTranslated = true;
                // FIX: When showing a cached translation, also ensure it's treated as expanded.
                postEntry.isExpanded = true;
                notifyDataSetChanged(); // Refresh the whole list to update button states and layout
            } else {
                translatingPermlinks.add(postEntry.permlink);
            ...
            postEntry.isTranslated = false;
            // When switching back to original, we don't change the isExpanded state.
            // The user might want to see the original text in its expanded form.
            renderContent(postEntry, body, progressBarBody);
            notifyDataSetChanged(); // Refresh to ensure correct text (short/long) is shown
        }
    }

    void renderContent(SingleHivePostModel postEntry, TextView body, RelativeLayout progressBarBody){
        String finalText;
        // This is the original logic from the file you sent, which works correctly
        // as long as the isExpanded and isTranslated flags are set properly.
        if (this.isComment || postEntry.isExpanded) {
            finalText = postEntry.body;
        } else {
            finalText = postEntry.shortBody;
        }

        if (postEntry.isTranslated && postEntry.translatedText != null && !postEntry.translatedText.isEmpty()){
            // If it's translated, ALWAYS show the full translated text.
    ...
    private Boolean isPaid(SingleHivePostModel postEntry){
        if (postEntry!=null){ return postEntry.is_paidout; } return false;
    }
    ...
    private String convertImageLinksToMarkdown(String text) {
        Pattern pattern = Pattern.compile("(https?://\\S+?\\.(jpg|jpeg|png|gif))"); Matcher matcher = pattern.matcher(text); StringBuffer modifiedText = new StringBuffer(); while (matcher.find()) { String imageUrl = matcher.group(); matcher.appendReplacement(modifiedText, "![](" + imageUrl + ")"); } matcher.appendTail(modifiedText); return modifiedText.toString();
    }

    private String convertYouTubeLinksToEmbed(String text) {
        Pattern pattern = Pattern.compile("(https?://(?:www\\.)?(youtube\\.com/(watch\\?v=|shorts/)|youtu\\.be/|you\\.tube/)([a-zA-Z0-9_-]+))"); Matcher matcher = pattern.matcher(text); StringBuffer modifiedText = new StringBuffer(); while (matcher.find()) { String videoId = matcher.group(4); String embedCode = "<div style='position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden;'><iframe style='position: absolute; top: 0; left: 0; width: 100%; height: 100%;' src='https://www.youtube.com/embed/" + videoId + "' frameborder='0' allowfullscreen></iframe></div>"; matcher.appendReplacement(modifiedText, embedCode); } matcher.appendTail(modifiedText); return modifiedText.toString();
    }
}
